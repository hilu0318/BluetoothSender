...
import com.test.hilu0318.bluetoothsender.adapter.ImageSelectAdapter;
...
public class ImageSelectActivity extends DataActivity {

    private ListView listView;
    private ImageSelectAdapter listAdapter;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setMode(Flag.FILE_SEND_MODE);
        setContentView(R.layout.img_select_layout);
        ...
        data = getImageDB();
        listAdapter = new ImageSelectAdapter(this, R.layout.img_select_list_item_layout, R.id.ad_img_filename, data);
    ...
    private List<ImageInfo> getImageDB(){
        List<ImageInfo> imgList = new ArrayList<>();

        String[] datalist =
                {MediaStore.Images.Media.DATA, MediaStore.Images.Media.DISPLAY_NAME,
                        MediaStore.Images.Media.ORIENTATION, MediaStore.Images.Media.SIZE, MediaStore.Images.Media.MIME_TYPE};

        CursorLoader cl =
                new CursorLoader(this, MediaStore.Images.Media.EXTERNAL_CONTENT_URI, datalist,
                        null, null, null);
        ...
        for(int j = 0; j<cs.getCount(); j++){
            ...
            imgInfo.setFilepath(cs.getString(index[0]));
            imgInfo.setFilename(cs.getString(index[1]));
            imgInfo.setOrientation(cs.getInt(index[2]));
            imgInfo.setFilesize(cs.getInt(index[3]));
            imgInfo.setMimetype(cs.getString(index[4]));
    ...
    public WriteObject makeData(ImageInfo info) {
